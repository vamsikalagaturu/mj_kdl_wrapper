...
static void lerp_q(const KDL::JntArray &a, const KDL::JntArray &b,
...
int main(int argc, char *argv[])
{
    ...
    cube.pos[0]   = kCubeX; cube.pos[1] = kCubeY; cube.pos[2] = kCubeZ;
    cube.rgba[0]  = 1.0f; cube.rgba[1] = 0.5f; cube.rgba[2] = 0.0f; cube.rgba[3] = 1.0f;
    cube.mass     = 0.1; cube.condim = 4;
    cube.friction[0] = 0.8; cube.friction[1] = 0.02; cube.friction[2] = 0.001;
    ...
    else              mj_kdl::sync_from_kdl(&robot, q_home);
    reset_cube(data);
    ...
    if (headless) {
        ...
        std::cout << "cube Z after pick: "
                  << std::fixed << std::setprecision(3) << cube_z << " m\n";
    } else {
