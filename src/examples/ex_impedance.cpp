...
int main(int argc, char *argv[])
{
    ...
    int                        key_id  = mj_name2id(env.model, mjOBJ_KEY, "home");
    if (!fingers) return 1;
     ...
     *   2. Compute jnt_trq_cmd from fresh *_msr
     *   3. Set the gripper command
     *   4. The next update() call applies the freshly computed commands.
     */
    auto step_impedance = [&]() {
        mj_kdl::update(&env); // refresh *_msr; previous jnt_trq_cmd is applied this step
