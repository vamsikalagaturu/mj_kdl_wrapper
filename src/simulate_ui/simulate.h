...
class Simulate {
 public:
  ...
  // Set wrapper-managed real-time factor displayed in the Simulation panel.
  void SetWrapperRealtimeFactor(double realtime_factor);
  int ConsumeWrapperRecordRequest(
      char* path, int path_size, int* camera, int* resolution, int* fps);
  void SetWrapperRecorderState(int state);
  ...
  const mjuiDef def_simulation[23] = {
    ...
    {mjITEM_STATIC,    "Rec",           2, nullptr,                "idle"},
    {mjITEM_END}
  };
