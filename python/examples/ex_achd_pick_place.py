...
def run_phase(scene, robot, chain, fk, achd, rnea, alpha, phase) -> bool:
    ...
    while scene.time() - t0 < phase["duration"]:
        t = smoothstep((scene.time() - t0) / phase["duration"])
...
def main() -> int:
    ...
    try:
        ...
        if args.gui:
            ...
            try:
                for phase in phases:
                    if not viewer.is_running() or not run_phase(
                        scene, robot, chain, fk, achd, rnea, alpha, phase
                    ):
                        break
            finally:
                viewer.close()
            ...
            for phase in phases:
                if not run_phase(scene, robot, chain, fk, achd, rnea, alpha, phase):
                    break
