...
To land work: branch off `dev`, open a PR into `dev`. To cut a release, follow the ordered checklist in [AGENTS.md](AGENTS.md) -- merges here are squash-only, so `main` must be merged back into `dev` afterwards or the next release opens with conflicts.
