...
1. **Check the version.** `cmake/Versions.cmake` (`MJ_KDL_VERSION`), `pyproject.toml`
   (`version`) and `package.xml` (`<version>`) must already read `X.Y.Z` -- `dev` carries the
   *next* version, bumped at step 6 of the previous release. Nothing to bump here;
   `scripts/check_versions.py` only checks that the three agree.
...
6. **Fast-forward `dev` to `main`, then bump.** Set all three version files to the next patch
